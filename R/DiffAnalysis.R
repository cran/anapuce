...
function (fileIN = "resNorm.txt", n = 3, ind.array = 1:2, name.A = "A", 
    ...
    varmixt.meth = FALSE, header = TRUE, sep = "\t", sep.write = "\t", dec.write = ".", ...) 
{
    ...
    if (!is.null(fileID)) {
        ...
        GenFic <- merge(fichier2,GenFic,by=names(fichier2)[names(fichier2) %in% names(GenFic)])      
    }
    indA <- match(paste(name.A, ind.array, sep = ""), names(GenFic))
    indM <- match(paste(name.M, ind.array, sep = ""), names(GenFic))
    if (dyeswap) {
    ...
    Int.Mean <- apply(GenFic[,indA],1, mean, na.rm = TRUE)
    LogRatio.Mean <- apply(GenFic[,indM],1, mean, na.rm = TRUE)
    All.Mean <- cbind(Int.Mean, LogRatio.Mean)
    tmp.LR <- GenFic[, indM]
    NbObs <- length(indM) - apply(1 * is.na(tmp.LR), 1, sum)
    good.NbObs<-which(NbObs>1)
    if (length(good.NbObs)<length(NbObs)) {
        tmp.LR <- GenFic[-good.NbObs, ]
        write.table(tmp.LR, file = fileDelete, row.names = FALSE,sep = sep.write, dec = dec.write, append = FALSE)
    } else {
        nom.colonne=names(GenFic)
        write.table(t(nom.colonne), file = fileDelete,row.names = FALSE, sep = sep.write, dec = dec.write,col.names = FALSE)
    }
    don <- GenFic[good.NbObs, ]
    All.Mean <- All.Mean[good.NbObs,]
    NbObs <- NbObs[good.NbObs]
    VarianceByGene <- apply(don[, indM],1,var,na.rm = TRUE)
    ...
    Delta <- apply(don[, indM], 1, mean, na.rm = TRUE)
    StatOfTest <- sqrt(NbObs) * (Delta/sqrt(VarianceByGene))
    PvalueRaw <- 2 * (1 - pt(abs(StatOfTest), NbObs - 1))
    PvalueAdj <-sapply(procs, function(meth) p.adjust(PvalueRaw, meth))
    PvalueAdj <- data.frame(don, All.Mean, VarianceByGene, StatOfTest,PvalueRaw,PvalueAdj)
    
    cat("##########################################################\n")
    cat("                  One variance by gene                    \n")
    cat("##########################################################\n")
 
        different <- apply(as.matrix(1:length(procs)), 1,FUN=function(x) PvalueAdj[which(PvalueAdj[,grep(procs[x],names(PvalueAdj))]<=alpha[x]),])
    ...
    if (length(GeneEnleve) != 0) {
        ...
        write.table(tmp.LR, file = fileDelete, row.names = FALSE,append = TRUE, col.names = FALSE, sep = sep.write, dec = dec.write)
        cat("Number of dropped genes : ", length(GeneEnleve), "\n")
        cat("Number of dropped genes with a too small variance : ",length(liste1), "\n")
        cat("Number of dropped genes with a too high variance : ",length(liste2), "\n")
    ...
    PvalueAdj <-sapply(procs, function(meth) p.adjust(PvalueRaw, meth))
    PvalueAdj <- data.frame(don2, All.Mean2, VarianceByGene=VarianceByGene2, StatOfTest=StatOfTest2, PvalueRaw,PvalueAdj)
    write.table(PvalueAdj, file = paste("Complete", fileOUT,sep = ""), row.names = FALSE, sep = sep.write, dec = dec.write)
    Sd2 <- sqrt(var(VarianceByGene2))
    cat(" Estimated variance (standard error): ", Variance2, "(", Sd2, ")","\n")
    different <- apply(as.matrix(1:length(procs)), 1,FUN=function(x) PvalueAdj[which(PvalueAdj[,grep(procs[x],names(PvalueAdj))]<=alpha[x]),])
    for (compteur in 1:length(different)){
        fichierRes <- different[[compteur]]
        cat("Multiple testing procedure : ", procs[compteur], 
        ...
        cat("Number of differentially expressed genes : ", nrow(fichierRes),"\n")
        if (nrow(fichierRes) != 0) 
            write.table(fichierRes, file = paste(procs[compteur], "-", alpha[compteur], "-", fileOUT, 
                sep = ""), row.names = FALSE, sep = sep.write, dec = dec.write)
    }
    if (varmixt.meth == TRUE) {
        ...
        resvm <- est.varmixt(VAR=VarianceByGene,Kmax=50,dfreedom=NbObs-1)
        groups <- as.vector(table(max.col(resvm$tau)))
        cat("----------------------------------------- \n")
        if (length(groups)==1) groups[1]=length(VarianceByGene)
        ...
        pval.tmp = apply(as.matrix(1:resvm$nmixt),1,FUN=function(x)  pnorm(abs(teststatVM), mean = 0, sd = sqrt(resvm$vars[x]/resvm$VM)) * resvm$tau[,x]) 
        pval.vm = apply(pval.tmp,1,sum) 
        }
        pval.vm <- 2 * (1 - pval.vm)
        resultatVM <-sapply(procs, function(meth) p.adjust(pval.vm, meth))
        PValueAdj <- data.frame(GenFic[good.NbObs,], Delta, VarianceByGene, NbObs,resvm$tau, VarianceVM=resvm$VM,StatOfTestVM=teststatVM,PValueVM=pval.vm,resultatVM)
        ...
        write.table(PValueAdj, file = paste("VM-Complete", 
            ...
            dec = dec.write)
        diffrtVM <- apply(as.matrix(1:length(procs)), 1,FUN=function(x) PValueAdj[which(PValueAdj[,grep(procs[x],names(PValueAdj))]<=alpha[x]),])
        for (compteur in 1:length(diffrtVM)){
        fichierResVM <- diffrtVM[[compteur]]
        cat("Multiple testing procedure : ", procs[compteur], 
            paste("(", alpha[compteur] * 100, "%)", sep = ""), 
            " \n")
        cat("Number of differentially expressed genes - VM : ", nrow(fichierResVM), 
            "\n")
         cat("-------------------------------------------------------------\n")
        if (nrow(fichierResVM) != 0) {
            names(fichierResVM) <- sub(name.M,"Delta",names(fichierResVM))
            write.table(fichierResVM, file = paste(procs[compteur], "-", alpha[compteur], "-VM-", fileOUT, 
                sep = ""), row.names = FALSE, sep = sep.write, dec = dec.write)
        }
    }
