      ...
      for (i in 1:niter.max) {
        ...
        if (i %in% c(2, 3)) {
          if (criterion == "parameter")
            crit <-
              max(abs((params.cur - params.old) / params.old))
          if (criterion == "likelihood")
            crit <-
              abs(log.lik.cur - log.lik.old) / (abs(log.lik.old))
          if (crit < stop.crit) {
            rm(gamma.dist)
            niter.f <- i
            vars <- b / 2
        ...
        if (i > 3) {
          ...
          eps <-
            params[[a]] + 1 / (1 / (params[[a - 1]] - params[[a]]) + 1 / (params[[a +
                                                                                    1]] - params[[a]]))
          ...
          if (max(abs(eps - old.eps)) <= stop.crit) {
            ...
            vars <-
              params[[a]][(nmixt + 1):length(params[[a]])] / 2
