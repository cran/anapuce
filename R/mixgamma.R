          ...
          display = TRUE, niter.max = 50000,criterion = criterion)
{
        ...
        for (i in 1:niter.max)
          {
            ...
            bmat <- t(as.matrix(b))
            gamma.dist <- apply(bmat, 2, FUN=function(sc, x, shape) dgamma(x = x, scale=sc, shape = shape),x = vec, shape = shap)
            ...
            p.i <- apply(ppost, 2, mean)
            b <- apply(ppost * (vec/shap), 2, sum)/(p.i * N)
            params.cur <- c(p.i, b)
            if (i > 1)
              {
                ...
                if (crit < stop.crit)
                  {
                    ...
                    vars <- b/2
                    loglike <- log.lik.cur
                    BIC.crit <- -2 * loglike + (2 * nmixt - 1) * log(N)
                    break
                  }
                if (i == niter.max)
                  {
    ...
    if (display) {
      cat("N mixt=", nmixt, ". EM algo, number of iterations=",
          niter.f, ". Elapsed time=", round(delta.t[3]/60,2), " minute(s).\n",
          sep = "")
      cat("pi=", p.i, "\n")
      cat("var=", vars, "\n")
    ...
    res <- list(BIC.crit = BIC.crit, p.i = p.i, vars = vars, loglike = loglike,
                nmixt = nmixt,tau=ppost)
