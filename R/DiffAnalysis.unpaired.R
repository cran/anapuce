DiffAnalysis.unpaired <-
            ...
            ...)
  {
    ...
    if ((nrow(GenFic) - length(don12)) > 0) {
      write.table(
        don12,
        file = "GenesOutOfAnalysis_NotEnoughObs.txt",
        row.names = FALSE,
        sep = "\t",
        append = FALSE
      )
    ...
    IndVarNulle <-
      GenFic[unique(c(which(VarByGene1 == 0), which(VarByGene2 ==
                                                      0))),]
    cat("--------------------------------------------------------------------- \n")
    ...
    if (length(IndVarNulle) != 0) {
      write.table(
        IndVarNulle,
        file = "GenesOutOfAnalysis_NullVar.txt",
        row.names = FALSE,
        ,
        append = FALSE,
        sep = "\t"
    ...
    tmp <-
      unique(c(which(NbObs1 < 2), which(NbObs2 < 2), c(
        which(VarByGene1 ==
    ...
    Moy1 <-
      rowMeans(GenFic[, grep(cond1, names(GenFic))], na.rm = TRUE)
    Moy2 <-
      rowMeans(GenFic[, grep(cond2, names(GenFic))], na.rm = TRUE)
    Delta <- Moy1 - Moy2
    pvalVartest = apply(
      as.matrix(1:nrow(GenFic)),
      1,
      FUN = function(x)
        bartlett.test(list(
          as.numeric(GenFic[x,
    ...
    PvalueRaw <- 2 * (1 - pt(abs(StatOfTest), (NbObs1 + NbObs2 -
                                                 2)))
    res <- sapply(procs, function(meth)
      p.adjust(PvalueRaw, meth))
    PvalueAdj <-
      data.frame(
        GenFic,
        Variance = PooledVar,
        VarianceByGene1 = VarByGene1,
        VarianceByGene2 = VarByGene2,
        StatOfTest,
        PvalueRaw,
        res
      )
    ...
    Moy1 <-
      rowMeans(GenFicm[, grep(cond1, names(GenFic))], na.rm = TRUE)
    Moy2 <-
      rowMeans(GenFicm[, grep(cond2, names(GenFic))], na.rm = TRUE)
    DeltaMoy <- Moy1 - Moy2
    ...
    PvalueAdj <-
      data.frame(
        GenFicm,
        VarianceByGene1 = VarByGene1[-unique(c(ou1p,
                                               ou1g, ou2p, ou2g))],
        VarianceByGene2 = VarByGene2[-unique(c(ou1p,
                                               ou1g, ou2p, ou2g))],
        StatOfTest = StatDeTest,
        PvalueRaw,
        res
      )
    write.table(
    ...
    cat(
      " Estimated variance (standard error) - condition 1: ",
      CommonVar1b[1],
      "(",
      sqrt(var(VarByGene1[-c(ou1p, ou1g)])),
      ")",
      "\n"
    )
    cat(
      " Estimated variance (standard error) - condition 2: ",
      CommonVar2b[1],
      "(",
      sqrt(var(VarByGene2[-c(ou2p, ou2g)])),
      ")",
      "\n"
    )
